...
def main():
    ...
    # Setup trackers for loss and average epoch loss
    losses_log = {
        "totalG": [],
        "totalD": [],
        "globalD": [],
    ...
    for epoch in range(EPOCH_NUM):
        ...
        for i, (masked, original, mask_known) in train_tqdm:
            ...
            # keep gp always in float32 for stability!
            ...
            #torch.cuda.empty_cache() removed to speedup
            ...
            with amp.autocast(device_type='cuda', dtype=torch.float16):
                ...
                # Multiscale L1 (Pixel-wise, under AMP)
                multi_l1 = 0.0
            ...
            # To avoid nan G and for stability
            scalerG.unscale_(optimizer_netG)
